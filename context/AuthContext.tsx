...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const updateUser = useCallback((updatedFields: Partial<UserProfile>) => {
    setUser(prev => {
      ...
      if (typeof window !== "undefined") {
        const isLocal = !!localStorage.getItem("auth_access_token");
        const s = isLocal ? localStorage : sessionStorage;
        s.setItem("auth_user", JSON.stringify(next));
      }
      return next;
    });
  }, []);
  ...
  return (
    <AuthContext.Provider
      value={{ user, loading, isAuthenticated: !!user, login, logout, logoutAll, updateUser }}
    >
      {children}
    </AuthContext.Provider>
  );
