      ...
      if (malichhocIds.length > 0) {
        ...
        if (mabuoihocIds.length > 0) {
          ...
          (diemdanhList ?? []).forEach((dd: any) => {
            ...
            const maphancong = (lichhocList ?? []).find(
              (l: any) => l.malichhoc === malichhoc,
            )?.maphancong;
            if (maphancong) {
          ...
          maphancongIds.forEach((id: number) => {
            ...
            tileChuyenCanMap[id] =
              stats.total > 0 ? stats.present / stats.total : null;
          });
        }
      }
  ...
  async getClassesData(magv: string) {
    ...
    if (maphancongIds.length > 0) {
      const { data: lichList } = await supabase
        .from("lichhoc")
        .select(
          "maphancong, thutrongtuan, tietbatdau, tietketthuc, phonghoc:maphong",
        )
        .in("maphancong", maphancongIds);
  ...
  async getAttendanceOverview(magv: string) {
    ...
    if (malichhocIds.length > 0) {
      ...
      const lichMap = new Map(
        lichhocList.map((l: any) => [l.malichhoc, l.maphancong]),
      );
    ...
    if (buoiHocList.length > 0) {
      ...
      const phancongMap = new Map(
        (phancongList ?? []).map((p: any) => [p.maphancong, p]),
      );
      ...
      dsDonXinNghi = (donList ?? []).map((don: any) => {
        ...
        return {
          ...
          class: pc
            ? `${pc.monhoc?.tenmon} - ${pc.lop?.tenlop}`
            : "Lớp học phần",
          dateRequested: bh ? bh.ngayhoc : "—",
          reason: don.lydo,
          dateSubmitted: don.ngaytao
            ? new Date(don.ngaytao).toISOString().slice(0, 10)
            : "—",
...
>>>>>>> Stashed changes
  ...
  async createAttendanceSession(maphancong: number, dateStr: string) {
    ...
    if (svMonHoc && svMonHoc.length > 0) {
      ...
      const leaveSet = new Set(
        approvedLeaves?.map((d: any) => d.masv?.trim()) || [],
      );
  ...
  async updateStudentAttendance(
    ...
    if (existing) {
      ...
      const { error: insertError } = await supabase.from("diemdanh").insert({
        mabuoihoc,
        masv: masvTrimmed,
        trangthai,
        ghichu: ghichu === "-" ? null : ghichu,
  ...
  async updateLeaveRequest(madon: number, status: string, magv: string) {
    ...
    const ddNote =
      trangthai === "DaDuyet" ? "Vắng có phép (Đơn xin nghỉ)" : null;
    ...
    if (existing) {
      ...
      await supabase.from("diemdanh").insert({
   ...
   */
  async studentQRCheckin(
    masv: string,
    mabuoihoc: number,
    qrSecretParam: string,
    lat: number,
    lng: number,
  ) {
    const cookieStore = await cookies();
    ...
    if (buoihoc.trangthai !== "DangDiemdanh") {
      throw new Error(
        "Phiên điểm danh này chưa được kích hoạt hoặc đã đóng lại.",
      );
    }
    ...
    const ROOM_COORDINATES: Record<string, { lat: number; lng: number }> = {
      ...
      B204: { lat: 21.00725, lng: 105.934001 },
    ...
    if (distance > 300) {
      throw new Error(
        `Bạn đang ở ngoài khu vực lớp học (cách ${Math.round(distance)}m). Vui lòng di chuyển vào trong lớp học (phạm vi 300m) để điểm danh!`,
      );
    }
...
>>>>>>> Stashed changes
  ...
  async saveGradeRow(
    ...
    maphancong: number,
    masv: string,
    grades: { loaidiem: string; giatri: number; heso: number }[],
  ) {
    // Lưu từng cột điểm
  ...
  async calculateAndSaveFinalGrade(maphancong: number, masv: string) {
    ...
    const diemTK = parseFloat(
      (
        (tongDiem / tongHeSo) * (tongHeSo < 1 ? tongHeSo : 1) +
        (tongHeSo < 1 ? 0 : 0)
      ).toFixed(2),
    );
    ...
    if (existing) {
      ...
      const { error } = await supabase.from("diemtongket").insert({
        masv: masvTrimmed,
        maphancong,
        diemtongket: diemFinal,
        diemchu: diemChu,
  ...
  async getRosterStudents(maphancong: number) {
    ...
    return (data ?? []).map((item: any) => {
      ...
      return {
        ...
        parent: sv
          ? `${sv.tenphuhuynh ?? "Chưa rõ"} - ${sv.sodienthoaiphuhuynh ?? "Chưa rõ"}`
          : "—",
        parentName: sv?.tenphuhuynh ?? "",
        parentPhone: sv?.sodienthoaiphuhuynh ?? "",
        address: sv?.diachithuongtru ?? "—",
  ...
  async getTasks(magv: string) {
    ...
    return (baitapList ?? []).map((bt: any) => {
      const pc = phancongList?.find(
        (p: any) => p.maphancong === bt.maphancong,
      ) as any;
...
=======
    const updatePayload: any = {
      ...
      ngaycapnhat: new Date(Date.now() + 7 * 60 * 60 * 1000)
        .toISOString()
        .replace("Z", ""),
    };
...
>>>>>>> Stashed changes
  ...
  async getExams(magv: string) {
    ...
    const { data: exams, error } = await supabase
      .from("dethi")
      .select("*, cauhoi(*, dapan(*)), ketquathi(*, sinhvien(hodem, ten))")
      .in("maphancong", maphancongIds)
    ...
    return (exams ?? []).map((exam: any) => {
      const pc = phancongList?.find(
        (p) => p.maphancong === exam.maphancong,
      ) as any;
      return {
        ...exam,
        tenlop: pc?.lop?.tenlop || "Không rõ",
        tenmon: pc?.monhoc?.tenmon || "Không rõ",
        ketquathi: exam.ketquathi?.map((kq: any) => ({
          ...kq,
          tensv: kq.sinhvien ? `${kq.sinhvien.hodem} ${kq.sinhvien.ten}` : "Không rõ",
        })) || [],
      };
    });
  },
  ...
  async endExam(magv: string, madethi: number) {
    ...
    const { data: examCheck } = await supabase
      .from("dethi")
      .select("madethi, thoigianbatdau, thoigianlam")
      .eq("madethi", madethi)
      .in("maphancong", maphancongIds)
      .maybeSingle();
    ...
    const now = new Date();
    const start = parseVNTime(examCheck.thoigianbatdau);

    // Tính số phút chênh lệch làm tròn LÊN để an toàn với DB
    const diffMinutes = Math.ceil((now.getTime() - start.getTime()) / 60000);
    
    // Đảm bảo tối thiểu là 1 phút
    const newThoigianlam = Math.max(1, diffMinutes);
    
    // Đảm bảo thời gian kết thúc LUÔN lớn hơn hoặc bằng thời gian bắt đầu + thoigianlam phút
    const safeEndTime = new Date(Math.max(now.getTime(), start.getTime() + newThoigianlam * 60000));

    // Chuyển sang chuỗi format local (VN time) để insert vào Supabase timestamp without time zone
    const vnSafeEndTime = new Date(safeEndTime.getTime() + 7 * 60 * 60 * 1000)
      .toISOString()
      .replace("Z", "");

    const updatePayload: any = {
      thoigianketthuc: vnSafeEndTime,
    };

    if (newThoigianlam < examCheck.thoigianlam) {
      updatePayload.thoigianlam = newThoigianlam;
    }
    ...
    // Tự động nộp bài cho sinh viên đang làm bài ('DangLam')
    const { error: studentUpdateErr } = await supabase
      .from("ketquathi")
      .update({
        trangthai: "HetGio",
        thoigiannopbai: vnSafeEndTime,
      })
      .eq("madethi", madethi)
      .eq("trangthai", "DangLam");

    if (studentUpdateErr) {
      console.warn("Lỗi cập nhật tự động nộp bài cho sinh viên khi dừng ca thi:", studentUpdateErr.message);
    }
  ...
  async createExamWithQuestions(
    ...
    assignedStudents: Array<{ masv: string; tensv: string }> = [],
  ) {
    const cookieStore = await cookies();
    const supabase = createClient(cookieStore);
...
>>>>>>> Stashed changes
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (questionType !== "TuLuan" && q.dapan.length > 0) {
        const dapanInserts = q.dapan.map((d, dIdx) => ({
          macauhoi: newQ.macauhoi,
          noidung: d.noidung,
          ladapandung: d.ladapandung,
    ...
    // Tạo thông báo cho sinh viên của lớp thi
    const { data: gvData } = await supabase
      .from("giangvien")
      .select("mataikhoan")
      .eq("magv", magv)
    ...
    if (gvData?.mataikhoan) {
      ...
      const targetText = studentCount
        ? ` ${studentCount} sinh viên trong danh sách dự thi`
        : " toàn bộ sinh viên lớp";

      await supabase.from("thongbao").insert({
        mataikhoantao: gvData.mataikhoan,
        tieude: `Bài thi mới: ${examData.tieude}`,
        noidung: `Giảng viên đã phát đề thi "${examData.tieude}"${targetText}. Thời gian làm bài ${examData.thoigianlam} phút.`,
        loai: "HocTap",
        doituong: "SinhVien",
        maphancong: examData.maphancong,
        ghim: false,
  ...
  async updateExamTime(
    ...
    if (!examCheck) {
      throw new Error(
        "Không tìm thấy đề thi hoặc bạn không có quyền chỉnh sửa",
      );
    }
...
>>>>>>> Stashed changes
  ...
  async getClassStats(magv: string, maphancong: number) {
    ...
    const avgGpa =
      totalStudents > 0 ? Number((totalGpa / totalStudents).toFixed(2)) : 0;
    const passRate =
      totalStudents > 0
        ? Number(((passCount / totalStudents) * 100).toFixed(1))
        : 0;
...
=======
          const present = ddRows.filter(
            (d: any) => d.trangthai === "Comat" || d.trangthai === "Dimuon",
          ).length;
>>>>>>> Stashed changes
...
=======
        A:
          totalStudents > 0
            ? Math.round((gradeDist.A / totalStudents) * 100)
            : 0,
        B:
          totalStudents > 0
            ? Math.round((gradeDist.B / totalStudents) * 100)
            : 0,
        C:
          totalStudents > 0
            ? Math.round((gradeDist.C / totalStudents) * 100)
            : 0,
        DF:
          totalStudents > 0
            ? Math.round(((gradeDist.D + gradeDist.F) / totalStudents) * 100)
            : 0,
      },
>>>>>>> Stashed changes
    };
...
>>>>>>> Stashed changes
    ...
    if (!tlCheck) {
      throw new Error(
        "Không tìm thấy báo cáo hoặc bạn không có quyền chỉnh sửa",
      );
    }
