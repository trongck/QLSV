/**
 * services/repositories/teacher-messages.repo.ts
 * Tầng truy cập dữ liệu cho tính năng Tin nhắn (Giảng viên).
 */
...
/** Lấy danh sách cuộc trò chuyện của giảng viên */
export async function getConversations(): Promise<ConversationRow[]> {
  const res = await apiFetch("/api/giangvien/messages/conversations");
...
export async function getMessages(
  ...
  const res = await apiFetch(`/api/giangvien/messages/conversations/${conversationId}?${q}`);
...
export async function sendMessage(
  ...
  const res = await apiFetch(`/api/giangvien/messages/conversations/${conversationId}`, {
    method: "POST",
...
export async function uploadAttachment(
  ...
  const res = await apiFetch("/api/giangvien/messages/upload", { method: "POST", body: form });
  if (!res.ok) {
...
export async function deleteConversation(conversationId: number): Promise<void> {
  const res = await apiFetch(`/api/giangvien/messages/conversations/${conversationId}`, { method: "DELETE" });
  await apiJson<{ success: boolean }>(res);
}

/** Xóa một tin nhắn cụ thể (ẩn với user) */
export async function deleteMessage(messageId: number): Promise<void> {
  const res = await apiFetch(`/api/giangvien/messages/${messageId}`, { method: "DELETE" });
  await apiJson<{ success: boolean }>(res);
}
...
export async function getConversationMembers(conversationId: number): Promise<ConversationMember[]> {
  const res = await apiFetch(`/api/giangvien/messages/conversations/${conversationId}/members`);
...
export async function searchUsers(query: string, limit = 10): Promise<UserSearchResult[]> {
  ...
  const res = await apiFetch(`/api/giangvien/messages/users?${q}`);
