...
export default function ExamPage() {
  ...
  // Nếu có examId trên URL → render view tương ứng, không dùng DashboardShell
  if (examIdParam) {
    const examId = parseInt(examIdParam, 10);
    if (view === "result") {
      return <ExamResultView examId={examId} />;
    }
    return <ExamSessionView examId={examId} />;
  }

  // Không có examId → hiển thị danh sách đề thi
  return <ExamList />;
}

// ─── Tách thành component con để giữ page.tsx gọn ─────────────────────────────
function ExamList() {
  const router = useRouter();
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4 flex-1 w-full">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-900 text-lg leading-tight">{exam.tieude}</h3>
                  ...
                  <div className="flex flex-col gap-1 w-full md:w-40">
                    ...
                    <span className="text-sm font-bold text-gray-800">{exam.thoigianlam} phút</span>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center w-full md:w-40">
                    <div className={`px-4 py-2 rounded-lg text-center w-full ${status.color} border border-current opacity-80`}>
                      <p className="text-[10px] uppercase font-black tracking-wider">{status.text}</p>
                    </div>
                  </div>
