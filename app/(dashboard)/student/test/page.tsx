...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    if (!authLoading && user && user.vaitro !== VaiTro.SinhVien)
...
=======
  const parseExamDate = (dateStr: string) => {
    ...
    let normalized = dateStr.trim().replace(/\s+/g, "T");
    const hasTimezone = /([zZ]|[+-]\d{2}:?\d{2})$/.test(normalized);
    if (!hasTimezone) {
      normalized = `${normalized}:00+07:00`;
    }
...
>>>>>>> Stashed changes
    const now = new Date();
    const start = parseExamDate(exam.thoigianbatdau);
    const end = parseExamDate(exam.thoigianketthuc);
                ...
                >
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        ...
                        {exam.matkhau && (
                          <div className="flex items-center gap-1 px-2 py-0.5 bg-yellow-50 text-yellow-600 text-[10px] font-bold rounded-full border border-yellow-100">
                            <Search size={10} className="rotate-45" /> Cần mật
                            khẩu
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex flex-col gap-1 w-full md:w-48">
                    ...
                    <span className="text-sm font-bold text-gray-800">
                      {parseExamDate(exam.thoigianbatdau).toLocaleString(
                        "vi-VN",
                        {
                  ...
                  <div className="flex flex-col items-center justify-center w-full md:w-40">
                    <div
                      className={`px-4 py-2 rounded-lg text-center w-full ${status.color} border border-current opacity-80`}
                    >
                  ...
                  <div className="w-full md:w-40">
                    {exam.trangthai === "DaLam" ? (
                      <div className="flex flex-col gap-2">
                        ...
                        <button
                          onClick={() =>
                            router.push(`/student/test/${exam.madethi}/result`)
                          }
                      ...
                      <button
                        disabled
                        className="w-full py-3 px-4 bg-gray-50 text-gray-400 rounded-xl text-sm font-bold cursor-not-allowed border border-gray-100"
                      >
                        {new Date() > new Date(exam.thoigianketthuc)
                          ? "Đã kết thúc"
                          : "Chưa đến giờ"}
                      </button>
                    )}
