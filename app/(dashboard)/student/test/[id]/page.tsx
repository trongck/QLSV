...
>>>>>>> Stashed changes
  ...
  // Ref để timer luôn gọi phiên bản mới nhất của handleSubmit (tránh stale closure)
  ...
  // Timer logic — chỉ khởi chạy sau khi exam đã được tải (exam !== null)
  useEffect(() => {
    if (loading || isFinished || showPasswordModal || !exam) return;

    // Chỉ auto-submit khi timeLeft đã về 0 sau khi đếm xuống (không submit ngay khi mới load)
    if (timeLeft <= 0) {
  ...
  }, [timeLeft, loading, isFinished, showPasswordModal, exam]);

  const updateExamSession = useCallback(
    async (
      action: "START" | "PROGRESS",
      answeredCount?: number,
      cheatCountValue?: number,
    ) => {
      if (!id) return;

      try {
        await apiFetch(`/api/student/exam/${id}`, {
          method: "PUT",
        ...
        console.warn("Không thể cập nhật phiên thi:", err);
      }
    },
    [id],
  );
  ...
  useEffect(() => {
    ...
    const handleBlur = () => {
      setCheatCount((prev) => {
        const next = prev + 1;
        if (sessionStarted) {
          updateExamSession("PROGRESS", Object.keys(answers).length, next);
        }
        return next;
      });
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const formattedAnswers = questions.map((q) => ({
        macauhoi: q.macauhoi,
        madapan: q.loaicauhoi !== "TuLuan" ? (answers[q.macauhoi] ?? null) : null,
        cautraloituluan: q.loaicauhoi === "TuLuan" ? (answers[q.macauhoi] ?? null) : null,
      }));
...
>>>>>>> Stashed changes
      ...
      if (json.success) {
        ...
        const message =
          json?.message ||
          json?.error ||
          "Nộp bài không thành công. Vui lòng thử lại.";
        setSubmissionError(message);
      }
    } catch (err: any) {
      console.error("Submit failed:", err);
      setSubmissionError(err?.message || "Đã xảy ra lỗi khi nộp bài.");
    } finally {
      setIsSubmitting(false);
    }
  };

  // Cập nhật ref mỗi lần handleSubmit thay đổi để timer luôn gọi phiên bản mới nhất
  React.useEffect(() => {
  ...
  useEffect(() => {
    if (loading || isFinished || showPasswordModal || !exam || sessionStarted) {
      return;
    }

    updateExamSession("START", Object.keys(answers).length, cheatCount);
  ...
  useEffect(() => {
    if (loading || isFinished || showPasswordModal || !sessionStarted) {
      return;
    }

    const answeredCount = Object.keys(answers).length;
    const timer = window.setTimeout(() => {
      updateExamSession("PROGRESS", answeredCount, cheatCount);
    }, 400);
          ...
          <div className="bg-gray-50 rounded-2xl p-6 mb-8 grid grid-cols-2 gap-4">
            ...
            <div className="text-center border-l border-gray-200">
              ...
              <p className="text-3xl font-black text-gray-800">
                {result?.socandung ?? 0}/{questions.length}
              </p>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-[#FAF7F6] flex flex-col">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex flex-col items-end gap-3">
          ...
          <div className="flex items-center gap-6">
            <div
              className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${timeLeft < 300 ? "bg-red-50 text-red-600 border-red-100 animate-pulse" : "bg-gray-50 text-gray-700 border-gray-100"}`}
            >
              <Clock size={18} />
              ...
              <div className="max-w-3xl mx-auto">
                ...
                <div className="space-y-3">
                  {currentQuestion.loaicauhoi === "TuLuan" ? (
                    <textarea
                      className="w-full min-h-50 p-6 rounded-2xl border-2 border-gray-100 focus:border-red-600 focus:ring-0 outline-none text-gray-700 transition-all resize-none"
                      placeholder="Nhập câu trả lời của bạn tại đây..."
                      value={answers[currentQuestion.macauhoi] || ""}
                      onChange={(e) =>
                        handleEssayChange(
                          currentQuestion.macauhoi,
                          e.target.value,
                        )
                      }
                    />
        ...
        <div className="w-full lg:w-80 flex flex-col gap-6">
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="mt-8 space-y-3">
              ...
              <div className="flex items-center gap-3 text-xs text-gray-400 font-bold">
                <div className="w-4 h-4 bg-red-50 border border-red-100 rounded-sm" />{" "}
                Đã trả lời
              </div>
              <div className="flex items-center gap-3 text-xs text-gray-400 font-bold">
                <div className="w-4 h-4 bg-white border border-gray-100 rounded-sm" />{" "}
                Chưa trả lời
              </div>
            </div>
          </div>
          ...
          <div className="bg-red-600 rounded-3xl p-6 text-white shadow-xl shadow-red-100">
            ...
            <p className="text-sm font-medium leading-relaxed">
              Hệ thống sẽ tự động nộp bài khi hết thời gian. Đừng quên nhấn "Nộp
              bài" sau khi hoàn thành.
            </p>
          </div>
        </div>
      </main>
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 text-center animate-scaleUp">
            ...
            <p className="text-gray-500 mb-8">
              Vui lòng nhập mật khẩu được cung cấp để bắt đầu bài thi.
            </p>
      ...
      {showSubmitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 text-center animate-scaleUp">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 ${unansweredCount > 0 ? "bg-yellow-100 text-yellow-600" : "bg-green-100 text-green-600"}`}
            >
              <AlertCircle size={32} />
            </div>
            ...
            {unansweredCount > 0 ? (
              <>
                ...
                <p className="text-gray-500 mb-8 leading-relaxed">
                  ...
                  câu hỏi chưa trả lời. Bạn có chắc chắn muốn nộp bài ngay bây
                  giờ không?
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-gray-500 mb-8 leading-relaxed">
                  Bạn đã trả lời hết tất cả các câu hỏi. Hãy kiểm tra kỹ lại một
