...
import { useMessages } from "@/hook/sinhvien/useMessages"; // Đảm bảo đường dẫn này đúng với thư mục của bạn
import {
  Search,
  Edit3,
  Phone,
...
export default function MessagesPage() {
  ...
  const { chatList, messages, isLoading, fetchChatRooms, fetchMessages, sendMessage } = useMessages();

  // 2. Các state dùng riêng cho giao diện này
  const [selectedChatId, setSelectedChatId] = useState<number | null>(null);
  const [inputText, setInputText] = useState("");

  const CURRENT_USER_ID = "SV22A005"; // Giả lập User ID của Lý Thị Kiều
